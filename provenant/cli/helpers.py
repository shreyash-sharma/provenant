...
from provenant.core.reasoning import (
    ReasoningMode,
)
from provenant.core.reasoning import (
    resolve_reasoning as resolve_core_reasoning,
)
from provenant.core.repo_config import CONFIG_FILENAME, load_repo_config
...
def find_workspace_root(start: Path | None = None) -> Path | None:
    ...
    Delegates to :func:`provenant.core.workspace.config.find_workspace_root`.
    """
    from provenant.core.workspace.config import find_workspace_root as _find

    return _find(start)


def get_provenant_dir(repo_path: Path) -> Path:
    """Return the ``.provenant/`` directory for a given repo root."""
    return repo_path / PROVENANT_DIR



def ensure_provenant_dir(repo_path: Path) -> Path:
    """Create the ``.provenant/`` directory if it does not exist and return it."""
    d = get_provenant_dir(repo_path)
...
def get_db_url_for_repo(repo_path: Path) -> str:
    ...
    Prefers ``PROVENANT_DB_URL``, then the legacy ``PROVENANT_DATABASE_URL``.
    Otherwise defaults to the repo-local ``<repo>/.provenant/wiki.db``.
    """
...
def load_state(repo_path: Path) -> dict[str, Any]:
    """Load ``.provenant/state.json`` or return an empty dict if absent."""
    state_path = get_provenant_dir(repo_path) / STATE_FILENAME
...
def save_state(repo_path: Path, state: dict[str, Any]) -> None:
    """Write *state* to ``.provenant/state.json``."""
...
# ---------------------------------------------------------------------------
# Update lock - coordinates concurrent `provenant update` invocations and
...
def _update_lock_path(repo_path: Path) -> Path:
    return get_provenant_dir(repo_path) / UPDATE_LOCK_FILENAME
...
def acquire_update_lock(repo_path: Path, target_commit: str | None) -> Path:
    ...
    ensure_provenant_dir(repo_path)  # ensure .provenant/ exists
...
def save_config(
    ...
    *,
    ...
    """Write provider/model/embedder (and optionally exclude_patterns) to ``.provenant/config.yaml``.
...
def resolve_provider(
    ...
    Resolution order:
      ...
      2. ``PROVENANT_PROVIDER`` env var
      3. ``.provenant/config.yaml`` (written by ``provenant init``)
      4. Auto-detect from API key env vars
    """
    from provenant.llm.providers import get_provider
    ...
    if provider_name is None:
        provider_name = os.environ.get("PROVENANT_PROVIDER")
    ...
    raise click.ClickException(
        "No provider configured. Use --provider, set PROVENANT_PROVIDER, "
...
def validate_provider_config(provider_name: str | None = None) -> list[str]:
    ...
    if provider_name:
        ...
        for name, env_vars in provider_env_vars.items():
            if name == "gemini":
                if os.environ.get("PROVENANT_PROVIDER") == "gemini" and not (
            ...
            if missing:
                ...
                explicitly_requested = os.environ.get("PROVENANT_PROVIDER") == name
...
# none), which produced the Phase A bug where ``provenant update`` from a
# workspace root errored with a misleading "No previous sync found" message
# and left a stray ``.provenant/`` directory behind.
#
...
class CommandTarget:
    ...
    def notice(self, console_obj: Console, *, command: str = "") -> None:
        ...
        if self.mode == "workspace":
            ws_root = self.ws_root.name if self.ws_root else "(unknown)"
...
def resolve_command_target(
    *,
    ...
    3. Explicit ``path`` argument:
       - If the path itself contains ``.provenant-workspace.yaml`` ->
    ...
    4. No ``path``, no flags -> start from cwd and:
       ...
       - If cwd has its own ``.provenant/state.json`` (i.e. it's a repo
    ...
    if workspace_flag or repo_alias is not None:
        ...
        if ws_root is None:
            raise WorkspaceNotFound(
                "No .provenant-workspace.yaml found at or above "
                f"{base_path}. Run 'provenant init <workspace-dir>' to "
    ...
    cwd_state = get_provenant_dir(base_path) / STATE_FILENAME
    if cwd_state.exists():
        ...
        return CommandTarget(
            ...
            repo_path=base_path,
            ws_root=upstream,
            reason="cwd has its own .provenant/state.json (cd-into-repo wins)",
