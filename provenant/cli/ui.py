"""Branding, theme constants, and interactive UI helpers for the provenant CLI."""
...
def print_phase_header(
    ...
    """Print a styled phase separator."""
    console.print()
    console.print(
        Rule(
...
_MODEL_PROMPT_REJECT = {"y", "yes", "n", "no", "true", "false"}

_MODEL_DISPLAY_LIMIT = 12

_GENERATION_MODEL_EXCLUDES = (
    "embedding",
    "embed",
    "whisper",
    "tts",
...
# ---------------------------------------------------------------------------
# .env persistence - save/load API keys in .provenant/.env
# ---------------------------------------------------------------------------


def load_dotenv(repo_path: Path) -> None:
    """Load ``<repo>/.provenant/.env`` into ``os.environ`` (without overwriting).

    Supports ``export KEY=value``, quoted values (``KEY="value"``, ``KEY='value'``),
    and inline comments (``KEY=value  # comment``).
    """
    env_file = repo_path / ".provenant" / ".env"
...
def _save_key_to_dotenv(repo_path: Path, env_var: str, value: str) -> None:
    """Append or update a key in ``<repo>/.provenant/.env``."""
    env_dir = repo_path / ".provenant"
    ...
    # Ensure .provenant/.env is gitignored
    _ensure_gitignored(repo_path)


def _ensure_gitignored(repo_path: Path) -> None:
    """Add ``.provenant/.env`` to ``.gitignore`` if not already present."""
    gitignore = repo_path / ".gitignore"
    pattern = ".provenant/.env"
    ...
    if gitignore.exists():
        ...
        if not content.endswith("\n"):
            content += "\n"
        content += f"\n# provenant API keys (local)\n{pattern}\n"
        gitignore.write_text(content, encoding="utf-8")
    else:
        gitignore.write_text(
            f"# provenant API keys (local)\n{pattern}\n",
            encoding="utf-8",
        )
...
def _provider_secret(provider: str) -> str | None:
    ...
    return os.environ.get(env_var or "") if env_var else None


def _looks_like_generation_model(model: str) -> bool:
    m = model.lower()
    if any(token in m for token in _GENERATION_MODEL_EXCLUDES):
        return False
    return any(m.startswith(prefix) for prefix in _GENERATION_MODEL_PREFIXES)


def _sort_models_for_docs(provider: str, models: list[str], default_model: str) -> list[str]:
    seen: set[str] = set()
    filtered = []
    for model in models:
    ...
    if not filtered:
        filtered = [m for m in models if m and m not in seen]

    preferred_fragments = {
        "gemini": ("flash-lite", "flash", "pro"),
        "openai": ("gpt-4.1", "gpt-4o-mini", "gpt-4o", "o4-mini", "o3-mini"),
        "anthropic": ("haiku", "sonnet", "opus"),
        "deepseek": ("flash", "chat", "reasoner"),
        "openrouter": ("flash", "haiku", "mini", "sonnet"),
        "ollama": ("llama3.2", "llama3.1", "qwen", "mistral"),
    }.get(provider, ())
    ...
    def rank(model: str) -> tuple[int, str]:
        if model == default_model:
            return (0, model)
        low = model.lower()
        for idx, fragment in enumerate(preferred_fragments, 1):
            if fragment in low:
                return (idx, model)
        return (len(preferred_fragments) + 1, model)

    return sorted(filtered, key=rank)


def _list_accessible_models(provider: str) -> list[str]:
    """Best-effort model listing for providers that expose it.

    Returns an empty list when the SDK/API cannot list models. Validation
    later still catches bad custom names.
    """
    ...
    try:
        ...
        if provider == "anthropic":
            ...
            client = Anthropic(api_key=secret, timeout=10.0)
            page = client.models.list()
            return sorted(m.id for m in getattr(page, "data", []) if getattr(m, "id", None))
        ...
        if provider == "ollama":
            ...
            base_url = (secret or "http://localhost:11434").rstrip("/")
...
def prompt_model_select(
    ...
    console.print(
        "  [dim]Smaller is fine: Provenant is calibrated for flash-lite / nano / "
        "haiku / 8B-class Ollama. Bigger models usually cost more without "
        "improving docs.[/dim]"
    )

    with console.status("  Checking models available to this key...", spinner="dots"):
        available = _list_accessible_models(provider)

    ranked = _sort_models_for_docs(provider, available, default_model)
    shown = ranked[:_MODEL_DISPLAY_LIMIT]

    if shown:
        console.print("  Available models for this key:")
        for idx, model in enumerate(shown, 1):
            suffix = " [dim](default)[/dim]" if model == default_model else ""
            console.print(f"    [{idx}] [{BRAND}]{model}[/{BRAND}]{suffix}")
        console.print("    [custom] Type another model id")
        default_choice = str(shown.index(default_model) + 1) if default_model in shown else "1"
        while True:
            raw = click.prompt("  Select model", default=default_choice).strip()
            lowered = raw.lower()
            if lowered in _MODEL_PROMPT_REJECT:
                console.print(
                    f"  [{WARN}]Model must be a model id or a number from the list. "
    ...
    console.print(
        f"  [dim]Could not list models. Press Enter to use the default: {default_model}[/dim]"
    )
    while True:
        model = click.prompt(f"  Model id", default=default_model).strip()
        if model.lower() in _MODEL_PROMPT_REJECT:
            console.print(
                f"  [{WARN}]Enter a model id (e.g. gpt-4o) or press Enter for default.[/]"
            )
            continue
        return model
...
def interactive_provider_select(
    ...
    *,
    ...
    if chosen not in detected:
        ...
        # If this is an OpenAI-compatible third-party key (not a real sk- key),
        # prompt for the base URL so validation routes to the right endpoint.
        if chosen == "openai" and not key.startswith("sk-"):
            console.print(
                "  [dim]This looks like a third-party OpenAI-compatible key "
                "(Fireworks, Yotta, Together, etc.).[/dim]"
            )
            base_url = click.prompt(
                "  Enter the provider base URL",
                default="",
                show_default=False,
                prompt_suffix=" (e.g. https://gateway.yottalabs.ai/api/maas): ",
            ).strip()
            if base_url:
                os.environ["OPENAI_BASE_URL"] = base_url
                if repo_path is not None and (repo_path / ".provenant" / ".env").exists():
                    _save_key_to_dotenv(repo_path, "OPENAI_BASE_URL", base_url)
                console.print(f"  [{OK}]OK Base URL set[/]")
                console.print()

    # --- model ---
    default_model = _PROVIDER_DEFAULTS.get(chosen, "")
    model = prompt_model_select(console, chosen, default_model, model_flag)
...
def _prompt_api_key(
    ...
    *,
    ...
    """Prompt for an API key, set env var, and optionally save to .provenant/.env.
    ...
    console.print(f"  [{OK}]OK Key set for this session[/]")

    # Offer to save for future runs
    if repo_path is not None:
        save = click.confirm(
            "  Save key to .provenant/.env for future runs? (auto-gitignored)",
            default=True,
        )
        if save:
            _save_key_to_dotenv(repo_path, env_var, key)
            console.print(f"  [{OK}]OK Saved to .provenant/.env[/]")
    console.print()
...
def interactive_advanced_config(
    ...
    result["follow_renames"] = click.confirm(
        "  Track files across git renamesx (slower but more accurate)",
        default=False,
    )
    ...
    result["test_run"] = click.confirm(
        "  Test runx (full ingestion; LLM page generation limited to top 10 files for quick validation)",
        default=False,
    )
...
def print_index_only_intro(console: Console, has_provider: bool = False) -> None:
    ...
    lines = [
        "  [green]OK[/] Parse all source files (AST)",
        "  [green]OK[/] Build dependency graph (PageRank, communities)",
        "  [green]OK[/] Index git history (hotspots, ownership, co-changes)",
        "  [green]OK[/] Detect dead code",
        "  [green]OK[/] Extract architectural decisions",
        "  [green]OK[/] Set up MCP server for AI assistants",
    ]
    if has_provider:
        lines.append(
            "  [green]OK[/] [dim]Decision extraction enhanced (provider key detected)[/dim]"
        )
    lines.append("")
...
def build_contextual_next_steps(
    *,
    ...
    if index_only:
        steps.append(("provenant mcp .", "start MCP server for AI assistants"))
        steps.append(("provenant init --provider gemini", "generate full documentation"))
    else:
        steps.append(("provenant mcp .", "start MCP server for AI assistants"))
        steps.append(("provenant search <query>", "search the generated wiki"))

    if dead_unreachable + dead_unused > 0:
        steps.append(
            ("provenant dead-code", f"explore {dead_unreachable + dead_unused} dead code findings")
        )

    if hotspot_count > 0 and top_hotspot:
        steps.append((f"provenant risk {top_hotspot}", "assess risk for top hotspot"))

    if decision_count > 0:
        steps.append(("provenant decisions", f"browse {decision_count} architectural decisions"))

    if not steps:
        steps.append(("provenant mcp .", "start MCP server for AI assistants"))
        steps.append(("provenant search <query>", "search the index"))
...
def interactive_repo_select(
    ...
    *repos* is a list of :class:`~provenant.core.workspace.scanner.DiscoveredRepo`.
    ...
    for idx, repo in enumerate(repos, 1):
        status = f"[{OK}]indexed[/]" if repo.has_provenant else "[dim]new[/dim]"
...
_PHASE_LABELS: dict[str, str] = {
    ...
    "graph.calls": "  Resolving call graph",
    "graph.metrics": "  Computing PageRank",
    "graph.communities": "  Detecting communities",
    "graph.flows": "  Tracing data flows",
    "git": "Reading git history",
    "co_change": "Computing co-change graph",
    "dead_code": "Detecting dead code",
    "decisions": "Extracting design decisions",
    "generation": "Generating wiki",
}


class RichProgressCallback:
    """Adapter that implements ``provenant.core.pipeline.ProgressCallback``
