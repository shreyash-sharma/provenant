"""``provenant doctor`` - health check for the wiki setup."""
...
def _run_repo_checks(repo_path: _DoctorPath, repair: bool) -> bool:
    ...
    checks.append(_check(".provenant/ directory", provenant_dir.exists(), str(provenant_dir)))
    ...
    provider_ok = False
    try:
        from provenant.llm.providers import list_providers
    ...
    from provenant.cli.helpers import validate_provider_config
    ...
    if db_ok and page_count > 0:
        try:
            ...
            async def _check_stores():
                ...
                from provenant.core.persistence.vector_store import (
                    LanceDBVectorStore,
                )
                ...
                # Check vector store
                vs_ids: set[str] = set()
                lance_dir = provenant_dir / "lancedb"
                if lance_dir.exists():
                    try:
    ...
    if db_ok:
        try:
            ...
            async def _check_coordinator():
                ...
                from provenant.core.persistence.coordinator import AtomicStorageCoordinator
    ...
    # Display
    table = Table(title=f"provenant Doctor - {repo_path.name}")
        ...
        async def _repair():
            ...
            if lance_dir.exists() and (missing_from_vector or orphaned_vector):
                try:
                    ...
                    # user can re-run `provenant reindex` for real embeddings
                    embedder = MockEmbedder()
...
def doctor_command(
    ...
    if not_indexed:
        ...
        console.print(
            "  Run [bold]provenant update --workspace[/bold] to index them."
        )
    ...
    *,
    ...
    Covers:
      ...
      - State drift between ``WorkspaceConfig.last_commit_at_index`` and
        each repo's ``.provenant/state.json``.
    ...
    """
    from provenant.core.workspace.update import (
        read_state_commit,
    ...
    for entry in ws_config.repos:
        ...
        elif (abs_path / ".provenant").is_dir() and not disk_commit:
            rows.append((
                entry.alias, "[yellow]WARN[/yellow]",
                "state.json missing or empty (run `provenant update`)",
            ))
            issues.append(f"{entry.alias}: missing state.json")
        else:
            rows.append((entry.alias, "[green]OK[/green]", entry.path))

    table = Table(title="provenant Workspace Doctor")
...
def _check_mcp_registered(ws_root: _DoctorPath) -> None:
    ...
    if found_paths:
        ...
        console.print(
            ...
            "`provenant hook install` to register.[/dim]"
        )
