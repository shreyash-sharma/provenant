"""``provenant workspace`` - manage multi-repo workspaces."""
...
if TYPE_CHECKING:
    from provenant.core.workspace.config import WorkspaceConfig
...
def _require_workspace(start: Path | None = None) -> tuple[Path, "WorkspaceConfig"]:  # type: ignore[name-defined]
    ...
    if ws_root is None:
        raise click.ClickException(
            ...
            "Run 'provenant init <workspace-dir>' to create a workspace."
        )
...
def workspace_list(path: str | None) -> None:
    ...
    from provenant.cli.helpers import get_provenant_dir
    ...
    for entry in ws_config.repos:
        ...
        provenant_dir = get_provenant_dir(abs_path)
...
def _query_repo_counts(repo_path: Path) -> tuple[int, int]:
    ...
    from provenant.cli.helpers import get_db_url_for_repo, get_provenant_dir
...
def workspace_add(
    ...
    """
    from provenant.core.workspace.config import RepoEntry
    ...
    console.print(f"[green]OK[/green] Added repo '{alias}' ({rel_path}) to workspace.")

    if not run_index:
        console.print(
            "[yellow]Skipping index[/yellow] (--no-index). "
            f"Run [bold]provenant update --repo {alias}[/bold] to index later."
        )
        return
...
def _resolve_docs_flag(
    *,
    ...
    Priority:
      ...
      3. Primary repo's ``.provenant/config.yaml`` has a provider -> docs ON,
         reusing the same provider settings.
      4. ``PROVENANT_PROVIDER`` env var or detectable API key -> docs ON.
    ...
    from provenant.cli.helpers import load_config
    ...
    env_provider = _os.environ.get("PROVENANT_PROVIDER")
    if env_provider:
        return True, None
    for key in (
        "ANTHROPIC_API_KEY",
...
def _run_index_for_repo(
    ...
    *,
    ...
    DB, writes ``.provenant/state.json`` (so ``provenant update`` knows the
    ...
    from provenant.core.pipeline.orchestrator import run_pipeline
    ...
    primary_cfg: dict = {}
    if primary is not None:
        from provenant.cli.helpers import load_config as _load_cfg
    ...
    try:
        ...
        console.print(
            f"  [green]OK[/green] {file_count} files, {symbol_count:,} symbols"
        )
    except Exception as exc:
    ...
    # so we get cost gating, cascading, and full parity with `provenant init`.
    generated_pages = 0
    ...
    if generate_docs and provider is not None:
        try:
            ...
            console.print(f"  [green]OK[/green] Generated {generated_pages} pages")
        except Exception as exc:
            console.print(f"  [yellow]Doc generation failed: {exc}[/yellow]")
            docs_skip_reason = f"generation error: {exc}"

    # Persist state.json so `provenant update` has a baseline commit.
    ...
    # `provenant update` runs don't have to re-prompt.
    if generate_docs and provider is not None:
        save_config(
            repo_path,
    ...
    if not state["docs_enabled"]:
        ...
        console.print(
            f"  Run [bold]provenant update --repo {alias} --docs[/bold] "
...
def _generate_docs_for_added_repo(
    *,
    ...
    from provenant.core.generation import (
        ContextAssembler,
        GenerationConfig,
        PageGenerator,
    )
    from provenant.core.ingestion import (
        ASTParser,
        FileTraverser,
        GraphBuilder,
    )
...
def workspace_remove(alias: str) -> None:
    ...
    The repo's .provenant/ directory is preserved; only the workspace
    entry is deleted.
    """
    ws_root, ws_config = _require_workspace(Path.cwd())
    ...
    ws_config.save(ws_root)
    console.print(f"[green]OK[/green] Removed repo '{alias}' from workspace.")
    ...
    console.print(
        f"  (Indexed data at {removed.path}/.provenant/ was [bold]not[/bold] deleted.)"
    )
...
def workspace_scan(path: str | None, yes: bool) -> None:
    ...
    from provenant.core.workspace.config import RepoEntry
    from provenant.core.workspace.scanner import scan_for_repos
    ...
    for repo in new_repos:
        indexed_marker = " [green](indexed)[/green]" if repo.has_provenant else ""
    ...
    for repo in new_repos:
        ...
        if do_add:
            ...
            entry = RepoEntry(path=rel_path, alias=alias)
            ws_config.add_repo(entry)
            console.print(f"  [green]OK[/green] Added '{alias}'.")
            added += 1
...
def workspace_set_default(alias: str) -> None:
    ...
    if previous_default and previous_default != alias:
        console.print(
            f"[green]OK[/green] Default repo changed from "
            f"'[dim]{previous_default}[/dim]' to '[bold]{alias}[/bold]'."
        )
    else:
        console.print(f"[green]OK[/green] Default repo set to '[bold]{alias}[/bold]'.")
