"""``provenant update`` - incremental wiki regeneration for changed files."""
...
def _workspace_update(
    ...
    *,
    ...
    """
    from provenant.core.workspace import check_repo_staleness, update_workspace
    ...
    console.print(f"[bold]provenant update[/bold] - workspace: {ws_root.name}")
    console.print()
    ...
    for entry in ws_config.repos:
        ...
        if not (abs_path / ".provenant").is_dir():
    ...
    from provenant.core.workspace import RepoUpdateResult
...
@click.option(
    ...
    help=(
        ...
        "the value of `docs_enabled` from .provenant/state.json is used "
        "(set during `provenant init`)."
    ),
)
def update_command(
    path: str | None,
    ...
    # Load saved API keys from .provenant/.env (won't overwrite existing env vars)
    ...
    if base_ref is None:
        ...
        if upstream_ws is not None:
            hint = (
                ...
                "Did you mean: provenant update --workspace?"
            )
        raise click.ClickException(
            f"No previous sync found for {repo_path}. "
            "Run 'provenant init' there first, or pass --since <ref>."
            + hint
        )
    ...
    # post-commit hook fires `provenant update` in the background, then a
    ...
    console.print(f"[bold]provenant update[/bold] - {repo_path}")
    console.print(f"Diffing [cyan]{base_ref[:8]}..{(head or 'HEAD')[:8]}[/cyan]")

    from provenant.core.ingestion import ChangeDetector
    ...
    from provenant.core.generation import ContextAssembler, GenerationConfig, PageGenerator
    ...
    if cascade_budget is None:
        from provenant.core.ingestion.change_detector import compute_adaptive_budget
    ...
    if index_only:
        ...
        async def _persist_index_only() -> None:
            ...
            async with get_session(sf) as session:
                ...
                # current graph build. Without this, ``provenant update``
                ...
                try:
                    from provenant.core.pipeline.persist import persist_graph_nodes
    ...
    # table - matching what `provenant init` already does. Without this,
    # `provenant costs` only ever reflects the initial index and shows $0 for
    ...
    try:
        from provenant.core.analysis.decision_extractor import DecisionExtractor
    ...
    async def _persist() -> None:
        ...
        try:
            if new_decision_markers:
                ...
                from provenant.core.persistence.crud import bulk_upsert_decisions
            ...
            if git_meta_map:
                from provenant.core.persistence.crud import recompute_decision_staleness
        ...
        if dead_code_report and dead_code_report.findings:
            try:
                ...
                from provenant.core.persistence.crud import save_dead_code_findings
    ...
    try:
        from provenant.cli.editor_setup import refresh_editor_project_files
    ...
    try:
        ...
        if ws_root is not None:
            from provenant.core.workspace import WorkspaceConfig
    ...
    try:
        from provenant.core.generation.report import GenerationReport, render_report
