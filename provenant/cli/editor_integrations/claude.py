...
from provenant.cli.helpers import get_db_url_for_repo, load_config, run_async
...
class ClaudeCodeSetup:
    ...
    def write_project_files(
        ...
        from provenant.cli.mcp_config import save_root_mcp_config
    ...
    def register_client(self, console_obj: Any, repo_path: Path) -> None:
        from provenant.cli.editor_integrations.claude_config import (
        ...
        if desktop:
            console_obj.print(f"  [green]OK[/green] Claude Desktop MCP registered ({desktop})")

        code = register_with_claude_code(repo_path)
        if code:
            console_obj.print(f"  [green]OK[/green] Claude Code MCP registered ({code})")

        hooks = install_claude_code_hooks()
        if hooks:
            console_obj.print(
                "  [green]OK[/green] Claude Code hooks registered (PreToolUse + PostToolUse)"
            )
...
def _prompt_claude_md_enabled(console_obj: Any) -> bool:
    ...
    from provenant.cli.ui import BRAND
...
def maybe_generate_claude_md(
    ...
    *,
    ...
    cfg = load_config(repo_path)
    if no_claude_md:
        # Persist opt-out so 'provenant update' respects it.
    ...
    try:
        with console_obj.status("  Generating .claude/CLAUDE.md...", spinner="dots"):
            run_async(_write_claude_md_async(repo_path))
        console_obj.print("  [green]OK[/green] .claude/CLAUDE.md updated")
