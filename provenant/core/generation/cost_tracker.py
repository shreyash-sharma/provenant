"""Runtime LLM cost tracking for provenant.

Tracks token usage and cost per session, and optionally persists rows to
the ``llm_costs`` table for historical reporting via ``provenant costs``.
"""
    ...
    *session_factory* (async SQLAlchemy sessionmaker) is supplied.
    ...
    async def _persist(
        ...
        *,
        ...
        try:
            from provenant.core.persistence.models import LlmCost
            from provenant.core.persistence import get_session
