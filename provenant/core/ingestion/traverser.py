"""File traversal for the provenant ingestion pipeline.
...
source file that should be documented.  It respects:
  ...
  2. .provenantIgnore (same syntax, user overrides)
...
_EMPTY_IGNORE_SPEC = pathspec.PathSpec.from_lines("gitwildmatch", ())
...
class FileTraverser:
    ...
    def __init__(
        ...
        *,
        ...
        extra_ignore_filename: str = ".provenantIgnore",
        ...
        # Per-directory .provenantIgnore cache: absolute dir path -> PathSpec.
        # Pre-seed root so it isn't read twice (we already have self._extra_ignore).
        self._dir_ignore_cache: dict[str, pathspec.PathSpec] = {
            str(self.repo_root): self._extra_ignore or _EMPTY_IGNORE_SPEC,
        }
    ...
    def _walk(self) -> Iterator[Path]:
        ...
        for dirpath, dirnames, filenames in os.walk(self.repo_root):
            ...
            # Load per-directory .provenantIgnore for subdirectory pruning.
    ...
    def _get_dir_ignore(self, dirpath: Path) -> pathspec.PathSpec:
        """Return the per-directory .provenantIgnore spec, loading and caching on first access."""
        key = str(dirpath)
        if key not in self._dir_ignore_cache:
            ...
            if ignore_file.exists():
                ...
                self._dir_ignore_cache[key] = pathspec.PathSpec.from_lines(
                    "gitwildmatch", lines
                )
            else:
                self._dir_ignore_cache[key] = _EMPTY_IGNORE_SPEC
    ...
    def _build_file_info(self, abs_path: Path) -> FileInfo | None:
        ...
        if self._extra_exclude.match_file(rel_str):
            ...
            return None
        # Per-directory .provenantIgnore: check filename against the parent dir's spec.
