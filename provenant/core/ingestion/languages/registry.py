...
class LanguageRegistry:
    ...
    def __init__(self, specs: tuple[LanguageSpec, ...] = _SPECS) -> None:
        ...
        # Precompute immutable aggregate views. These are used by traversal,
        # parsing, CLI pre-scan, and generation setup during import/startup.
        self._all_specs: tuple[LanguageSpec, ...] = tuple(specs)
        self._all_code_extensions = frozenset(
            ext for spec in specs if spec.is_code for ext in spec.extensions
        )
        self._code_languages = frozenset(
            spec.tag for spec in specs if spec.is_code and not spec.is_passthrough
        )
        self._config_languages = frozenset(spec.tag for spec in specs if not spec.is_code)
        self._passthrough_languages = frozenset(spec.tag for spec in specs if spec.is_passthrough)
        self._infra_languages = frozenset(spec.tag for spec in specs if spec.is_infra)
        self._entry_point_names = frozenset(
            name for spec in specs for name in spec.entry_point_patterns
        )
        self._manifest_filenames = frozenset(
            name for spec in specs for name in spec.manifest_files
        )
        self._blocked_dirs = frozenset(name for spec in specs for name in spec.blocked_dirs)
        self._generated_suffixes = frozenset(
            suffix for spec in specs for suffix in spec.generated_suffixes
        )
