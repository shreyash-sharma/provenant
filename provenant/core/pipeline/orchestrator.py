"""Programmatic pipeline orchestrator for provenant.

Provides ``run_pipeline()`` - the single entry point for running the full
provenant indexing/analysis/generation pipeline without any CLI dependencies.

This module has **zero** imports from ``provenant.cli``, ``click``, or ``rich``.
...
from provenant.core.pipeline.progress import ProgressCallback
...
def _parse_one(path_and_fi_and_bytes: tuple) -> Any:
    ...
    try:
        if _WORKER_PARSER is None:
            from provenant.core.ingestion import ASTParser
...
async def run_pipeline(
    ...
    *,
    ...
    """Run the provenant indexing/analysis/generation pipeline.
    ...
    generation_config:
        ...
        ``.provenant/config.yaml`` and ``PROVENANT_REASONING``.
    ...
    if generate_docs and llm_client is not None:
        ...
        if resolved_generation_config is None:
            ...
            from provenant.core.reasoning import resolve_reasoning
            from provenant.core.repo_config import load_repo_config
...
async def _run_ingestion(
    ...
    *,
    ...
    try:
        from provenant.core.ingestion.tsconfig_resolver import TsconfigResolver
    ...
    if progress:
        progress.on_message(
            ...
            "Ctrl-C, then run 'provenant init --resume' to continue)",
        )
    ...
    try:
        from provenant.core.ingestion.dynamic_hints import HintRegistry
    ...
    if progress:
        ...
        if stats.skipped_extra_ignore:
            parts.append(f"{stats.skipped_extra_ignore:,} by .provenantIgnore")
