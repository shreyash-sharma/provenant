"""Vector store abstraction and implementations for provenant semantic search.
...
LanceDBVectorStore
    ...
    ``provenant-core[search]`` extra (lancedb>=0.12).

PgVectorStore
    Stores embeddings in the ``wiki_pages.embedding`` pgvector column.
    Requires the ``provenant-core[pgvector]`` extra and a running PostgreSQL
...
class VectorStore(ABC):
    ...
    async def list_page_ids(self) -> set[str]:
        ...
        Used by ``provenant doctor --repair`` to detect three-store
...
class LanceDBVectorStore(VectorStore):
    ...
    Requires the ``provenant-core[search]`` extra:
        pip install provenant-core[search]

    Data is stored in *db_path* (e.g. ``.provenant/lancedb/``).
    ...
    async def _ensure_connected(self) -> None:
        ...
        try:
            ...
            raise RuntimeError(
                "LanceDB is not installed. Install it with: pip install provenant-core[search]"
            ) from exc
...
class PgVectorStore(VectorStore):
    ...
    - The ``provenant-core[pgvector]`` extra.
